    ...
    Scout = class {
      static run(creep) {
        try {
          ...
          const phaseEmoji = memory.scoutingPhase === "moving" ? "\u27A1\uFE0F" : memory.scoutingPhase === "exploring" ? "\u{1F50D}" : "\u{1F3E0}";
      ...
      static exploreRoom(creep, currentRoomName) {
        ...
        if (!memory.lastExplored) {
          ...
          this.gatherRoomIntelligence(room, false);
        }
        ...
        if (distanceToCenter > 3) {
          ...
          if (explorationTime >= 5) {
            ...
            this.gatherRoomIntelligence(room, true);
            memory.scoutingPhase = "returning";
      ...
      static gatherRoomIntelligence(room, updateLastScouted = true) {
        try {
          if (!Memory.rooms[room.name]) {
            Memory.rooms[room.name] = {
              sources: {},
      ...
      static calculateRemoteScore(scoutData) {
        var _a, _b, _c;
        let score = 0;
        score += (((_a = scoutData.sources) == null ? void 0 : _a.length) || 0) * 30;
        score -= (scoutData.hostileCount || 0) * 20;
        if (scoutData.hasHostileStructures) score -= 50;
        if ((_b = scoutData.controller) == null ? void 0 : _b.owner) score -= 100;
        if (((_c = scoutData.controller) == null ? void 0 : _c.reservation) && scoutData.controller.reservation.username === "YourUsername") {
          score += 20;
        }
  ...
  runCreepRole(creep) {
    ...
    switch (creep.memory.role) {
      ...
      case "upgrader":
        ...
        break;
      case "scout":
        const { Scout: Scout2 } = (init_Scout(), __toCommonJS(Scout_exports));
        Scout2.run(creep);
        break;
      default:
